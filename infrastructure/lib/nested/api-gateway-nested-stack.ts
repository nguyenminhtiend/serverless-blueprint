...
export interface ApiGatewayNestedStackProps extends BaseNestedStackProps {
...
export class ApiGatewayNestedStack extends BaseNestedStack {
  ...
  constructor(scope: Construct, id: string, props: ApiGatewayNestedStackProps) {
    ...
    this.jwtAuthorizer = new apigatewayv2Authorizers.HttpJwtAuthorizer(
      ...
      {
        authorizerName: `${this.environment}-cognito-jwt-authorizer`,
    ...
    const logGroup = new logs.LogGroup(this, 'ApiGatewayLogGroup', {
      logGroupName: `/aws/apigateway/${this.environment}-${this.projectName}-api`,
      retention:
        this.environment === 'prod' ? logs.RetentionDays.ONE_MONTH : logs.RetentionDays.THREE_DAYS,
      removalPolicy: this.environment === 'prod' ? cdk.RemovalPolicy.RETAIN : cdk.RemovalPolicy.DESTROY,
    });

    // Create HTTP API Gateway
    this.httpApi = new apigatewayv2.HttpApi(this, 'HttpApi', {
      apiName: `${this.environment}-${this.projectName}-api`,
      description: `${this.projectName} API - ${this.environment} environment`,
      corsPreflight: {
        allowCredentials: this.environment === 'prod' ? true : false,
  ...
  private createCloudWatchAlarms() {
    ...
    new cdk.aws_cloudwatch.Alarm(this, 'Api4XXErrorAlarm', {
      alarmName: `${this.environment}-api-gateway-4xx-errors`,
    ...
    new cdk.aws_cloudwatch.Alarm(this, 'Api5XXErrorAlarm', {
      alarmName: `${this.environment}-api-gateway-5xx-errors`,
    ...
    new cdk.aws_cloudwatch.Alarm(this, 'ApiLatencyAlarm', {
      alarmName: `${this.environment}-api-gateway-latency`,
