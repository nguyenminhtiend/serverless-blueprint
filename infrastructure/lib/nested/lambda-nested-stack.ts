...
export interface LambdaNestedStackProps extends BaseNestedStackProps {
...
export class LambdaNestedStack extends BaseNestedStack {
  ...
  constructor(scope: Construct, id: string, props: LambdaNestedStackProps) {
    ...
    const commonLambdaProps = {
      ...
      environment: {
        ...
        LOG_LEVEL: this.environment === 'prod' ? 'WARN' : 'INFO',
        POWERTOOLS_SERVICE_NAME: this.projectName,
        POWERTOOLS_LOG_LEVEL: this.environment === 'prod' ? 'WARN' : 'INFO',
        POWERTOOLS_LOGGER_SAMPLE_RATE: this.environment === 'prod' ? '0.1' : '1',
    ...
    this.authFunction = new nodejs.NodejsFunction(this, 'AuthFunction', {
      ...commonLambdaProps,
      functionName: `${this.environment}-auth-service`,
      ...
      logGroup: new logs.LogGroup(this, 'AuthFunctionLogGroup', {
        logGroupName: `/aws/lambda/${this.environment}-auth-service`,
    ...
    this.orderFunction = new nodejs.NodejsFunction(this, 'OrderFunction', {
      ...commonLambdaProps,
      functionName: `${this.environment}-order-service`,
      entry: '../packages/service-orders/src/index.ts',
      description: 'Order management service',
      logGroup: new logs.LogGroup(this, 'OrderFunctionLogGroup', {
        logGroupName: `/aws/lambda/${this.environment}-order-service`,
        retention: logRetention,
        removalPolicy: cdk.RemovalPolicy.DESTROY,
      }),
      environment: {
        ...commonLambdaProps.environment,
        EVENT_BUS_NAME: eventBusName || `${this.projectName}-events-${this.environment}`,
      },
    });
    ...
    this.notificationFunction = new nodejs.NodejsFunction(this, 'NotificationFunction', {
      ...commonLambdaProps,
      functionName: `${this.environment}-notification-service`,
      ...
      logGroup: new logs.LogGroup(this, 'NotificationFunctionLogGroup', {
        logGroupName: `/aws/lambda/${this.environment}-notification-service`,
        retention: logRetention,
        removalPolicy: cdk.RemovalPolicy.DESTROY,
      }),
      environment: {
    ...
    this.orderFunction.addToRolePolicy(
      new iam.PolicyStatement({
        ...
        resources: [
          `arn:aws:events:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:event-bus/${eventBusName || `${this.projectName}-events-${this.environment}`}`,
        ],
      })
    );
    ...
    this.notificationFunction.addToRolePolicy(
      new iam.PolicyStatement({
        ...
        resources: [`arn:aws:sqs:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:*${this.environment}*`],
      })
    );
    ...
    const cloudWatchLogsPolicy = new iam.PolicyStatement({
      ...
      resources: [
        `arn:aws:logs:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:log-group:/aws/lambda/${this.environment}-*`,
      ],
    });
    ...
    new cdk.CfnOutput(this, 'NotificationFunctionArn', {
      ...
      exportName: `${this.environment}-notification-function-arn`,
    });
  ...
  private createCloudWatchAlarms() {
    ...
    functions.forEach(({ func, name }) => {
      ...
      new cdk.aws_cloudwatch.Alarm(this, `${name}ErrorAlarm`, {
        alarmName: `${this.environment}-${name.toLowerCase()}-service-errors`,
      ...
      new cdk.aws_cloudwatch.Alarm(this, `${name}DurationAlarm`, {
        alarmName: `${this.environment}-${name.toLowerCase()}-service-duration`,
      ...
      new cdk.aws_cloudwatch.Alarm(this, `${name}ThrottleAlarm`, {
        alarmName: `${this.environment}-${name.toLowerCase()}-service-throttles`,
